# -*- coding: utf-8 -*-
"""
FiLM-conditioned iterative multi-mutation inference (应用版，含三对示例)
-------------------------------------------------------------------
- 读取你的三份 antigen 口袋 PDB（若无 npz 自动解析生成）
- 使用你给的 CombinedCDRs 作为示例序列
- 选择突变位点策略（all_nonX / first_n / custom）
- 以 Graph Transformer 口袋编码 + token-level FiLM 条件化的 MLM 方式逐位点迭代突变
- 详细日志（每轮选择、位置诊断、联合分布 Top-N、前后总结）
...
def parse_pdb_atoms(pdb_path: Path):
    ...
    with open(pdb_path, "r") as f:
        for line in f:
            ...
            elem.append(element.upper())
            resn.append(res_name.upper())
            resid.append(res_seq)
            chain.append(ch)
            atomn.append(atom_name.upper())
    if not xs:
        return None
...
def build_residue_graph_from_npz(npz_path: str, knn_k: int = 12):
    with np.load(npz_path, allow_pickle=True) as t:
        pos_atom = t["pos"]             # (N,3)
        res_name = t["res_name"]        # (N,)
        res_id   = t["res_id"]          # (N,)
        chain_id = t["chain_id"]        # (N,)
        atom_name= t["atom_name"]       # (N,)
...
class PocketGraphTransformer(nn.Module):
    def __init__(self, node_dim=21, hidden=256, layers=4, out_dim=256,
                 num_heads=8, dropout=0.0, use_knn_mask=True):
        super().__init__()
    ...
    def forward(self, g: dict) -> torch.Tensor:
        ...
        if x.size(0) == 0:
            return torch.zeros(self.readout[-1].out_features, device=dev, dtype=torch.float32)
        ...
        cls_h = h[:, 0, :]
        cond = self.readout(cls_h).squeeze(0)
        return cond

class FiLMToken(nn.Module):
    def __init__(self, cond_dim: int, d_model: int):
        super().__init__()
        self.fc = nn.Linear(cond_dim, 2*d_model)
        self.d_model = d_model
        with torch.no_grad():
            nn.init.zeros_(self.fc.weight)
            self.fc.bias[:d_model].fill_(1.0)
            self.fc.bias[d_model:].zero_()
    def forward(self, cond: torch.Tensor, H: torch.Tensor):
        gb = self.fc(cond)
...
class ProteinMLMContrastCond_Infer(nn.Module):
    """
    仅推理所需子集：ESM2 + lm_head（tie） + PocketGraphTransformer + token-level FiLM
    forward(seqs, mask_pos, graphs) -> seq_logits
    """
    def __init__(self, local_model_dir: str, cond_dim=256, proj_dim=128, use_knn_mask=True):
        super().__init__()
        ...
        self.film   = FiLMToken(cond_dim=cond_dim, d_model=d_model)
    ...
    def forward(self, seqs: List[str], mask_pos: List[List[int]], graphs: List[dict]):
        # cond from antigen graphs
        dev = next(self.parameters()).device
        cond_list = []
        with torch.cuda.amp.autocast(enabled=False):
            for g in graphs:
                cond_list.append(self.pocket(g))
        ...
        Hm = self.film(cond, H_masked)                # token-level FiLM
...
class FiLMIterativeMutator:
    def __init__(self, weights_path: str, local_model_dir: str, device: Optional[str] = None):
        ...
        # 加载训练好的权重（strict=False 以兼容投影头等缺失/多余）
        try:
    ...
    def mutate_iterative_one(self,
        ...
        for t in range(1, steps + 1):
            ...
            if not valid:
                if verbose: print(">> No valid positions remain; stop.")
                break
            ...
            if not joint_p:
                ...
                if float(row.sum().item()) == 0.0:
                    if not include_original_as_candidate and isinstance(orig_tid, int) and 0 <= orig_tid < row.numel():
                        row2 = probs_all[pos_idx].clone(); row2[orig_tid] = 0.0
...
# 生成的 npz 存放目录（若不存在会自动创建）
ANTIGEN_NPZ_DIR = Path("data/antigens")
ANTIGEN_NPZ_DIR.mkdir(parents=True, exist_ok=True)

# 预训练权重（本地 ESM2 模型目录 + 你训练好的 conditional 模型 best.pt）
LOCAL_MODEL_DIR = "../RandomMutation/models/esm2_650m"  # ← 按你的环境改
WEIGHTS_PATH    = "runs_cond_film_token_item_split_v1/best.pt"   # ← 你的 best.pt

# 突变位点策略（放在接口区，便于和上面数据一起改）
#  - "all_nonX"：所有非 'X' 的位置都算可突变
#  - "first_n" ：每条序列只取前 N 个可突变位点（见 FIRST_N）
#  - "custom"  ：用下面 CUSTOM_POSITIONS 指定每个 key 的位点列表（0-based）
POSITION_STRATEGY = "all_nonX"   # {"all_nonX","first_n","custom"}
FIRST_N = 8
CUSTOM_POSITIONS: Dict[str, List[int]] = {
    # 例： "6cbp": [31,33,52,57,60,98],
...
# 迭代突变 / 采样超参
K_MUTANTS            = 2       # 每条序列生成几个变体
TEMPERATURE          = 1.0     # softmax 温度
TOP_K                = 8       # 只保留概率 Top-K 的氨基酸（None 表示不启用）
TOP_P                = None    # nucleus 采样阈值（0~1，None 表示不启用）
AA_BLACKLIST         = "C"     # 不允许采样的氨基酸，空字符串表示不限

# 位置权重策略（影响“选哪个位点先改”）
SCHEME               = "combo3"  # {"uniform","anti_porig","entropy","margin_inverse","combo","combo3"}
ALPHA                = 1.0       # 熵项（不确定性）权重
BETA                 = 1.0       # (1 - p_orig) 项权重
GAMMA                = 0.7       # (1 - margin) 项权重

# 是否强制突变（不允许保持原氨基酸）
FORCE_MUTATE         = True      # True=强制改变原字母；False=允许不变
...
def main_run():
    ...
    for k in key2graph:
        # 放到同一设备张量在 forward 内部自动搬运；此处保持 CPU 即可
        pass
